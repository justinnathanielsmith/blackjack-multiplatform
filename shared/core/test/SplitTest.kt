...
class SplitTest {
    ...
    @Test
    fun split_createsTwoHands() = runTest {
        ...
        assertEquals(2, state.playerHands.getOrNull(1)?.cards?.size)
        assertEquals(Rank.EIGHT, state.playerHands[0].cards[0].rank)
        assertEquals(Rank.EIGHT, state.playerHands.getOrNull(1)?.cards?.get(0)?.rank)
        assertEquals(1, state.deck.size) // started with 3, used 2
    }

    @Test
    fun split_deductsBalance() = runTest {
    ...
    @Test
    fun split_invalid_nonPair() = runTest {
        val initialState = playingState(
    ...
    @Test
    fun split_invalid_insufficientBalance() = runTest {
        val initialState = playingState(
            balance = 50,
            playerHand = hand(Rank.EIGHT, Rank.EIGHT),
            dealerHand = hand(Rank.TEN, Rank.SEVEN),
    ...
    @Test
    fun split_invalid_afterHit() = runTest {
        val initialState = playingState(
    ...
    @Test
    fun split_invalid_whenAlreadySplit() = runTest {
    ...
    @Test
    fun splitAce_noExtraHit() = runTest {
        ...
        val initialState = GameState(
            ...
            currentBet = 100,
            playerHands = persistentListOf(
                Hand(persistentListOf(card(Rank.ACE), card(Rank.FIVE)), wasSplit = true, isFromSplitAce = true),
                Hand(persistentListOf(card(Rank.ACE), card(Rank.THREE)), wasSplit = true, isFromSplitAce = true),
            ),
            playerBets = persistentListOf(100, 100),
            activeHandIndex = 0,
            dealerHand = hand(Rank.TEN, Rank.SEVEN),
    ...
    @Test
    fun splitAce_autoStandsAfterDeal() = runTest {
        val initialState = playingState(
    ...
    @Test
    fun split_independentPayouts_primaryWinsSplitLoses() = runTest {
    ...
    @Test
    fun split_independentPayouts_bothWin() = runTest {
    ...
    @Test
    fun split_insertsHandAtActiveIndexPlusOne() = runTest {
    ...
    @Test
    fun splitAce_turnProgression() = runTest {
    ...
    @Test
    fun bust_onPrimaryHand_advancesToSplit() = runTest {
