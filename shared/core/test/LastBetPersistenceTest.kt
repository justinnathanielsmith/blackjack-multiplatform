...
class LastBetPersistenceTest {
    ...
    fun testMultiHandBetPersistence() = runTest {
        ...
        assertEquals(listOf(0, 0, 0), stateMachine.state.value.playerHands.map { it.bet })
        ...
        val previousBets = stateMachine.state.value.playerHands.map { it.bet }.toPersistentList()
